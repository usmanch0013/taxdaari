...
* notamment le widget 'Post grid'
*
* Méthode 'get_post_args'
* Méthode 'get_user_filters'
* Méthode 'get_meta_query_filters'
* Méthode 'get_taxo_tag_filters'
* Méthode 'compare_meta_values'
* Méthode 'get_meta_query_list'
* Méthode 'set_posts_query_args'
* Méthode 'get_posts_query_args'
* 
* @since 0.0.9
...
class Eac_Helpers_Util {
				...
				if(!empty($item['al_content_metadata_values'])) {
					...
					foreach($metadatasvalues as $metadatavalue) {
						...
						if($type === 'DATE') {
							...
							if($value === 'today') {
							...
							} else if($value === 'today-1m') {
								$value = date_i18n('Y-m-d', strtotime("-1 month"));
							} else if($value === 'today-1q') {
								$value = date_i18n('Y-m-d', strtotime('-3 month'));
							} else if($value === 'today-1y') {
								$value = date_i18n('Y-m-d', strtotime('-1 year'));
							}else if($value === 'today+1m') {
								$value = date_i18n('Y-m-d', strtotime("+1 month"));
							} else if($value === 'today+1q') {
								$value = date_i18n('Y-m-d', strtotime('+3 month'));
								...
								$value = date_i18n('Y-m-d', strtotime('+1 year'));
							} else { // Conversion de la date
								$value = Eac_Tools_Util::set_meta_value_date($value);
							}
	...
	public static function get_user_filters($which_user) {
		...
		$html .= "<div id='al-filters__wrapper' class='al-filters__wrapper'>";
			$html .= "<div class='al-filters__item al-active'><a href='#' data-filter='*'>" . __('Tous', 'eac-components') . "</a></div>";
		...
		$html .= "<div id='al-filters__wrapper-select' class='al-filters__wrapper-select'>";
			$html .= "<select class='al-filter__select'>";
				$html .= "<option value='*' selected>" . __('Tous', 'eac-components') . "</option>";
				foreach($which_users as $id_user) {
					$disp_user = get_user_by('id', trim($id_user));
					if($disp_user != false) {
	 ...
	 *
	 * @param {$taxonomy_filters}	Array: Un tableau de catégories
	 * @param {$slug_filters}		Array: Un tableau de slug des étiquettes
	 *
	 * @return les filtres des catégories formatées en HTML
	 * @since 1.6.0
	 * @since 1.7.0	Simplification du code
	 */
	public static function get_taxo_tag_filters($taxonomy_filters, $slug_filters) {
		$html = '';
		$unique_terms = array();
		// Récupère les étiquettes relatives à la taxonomie
		$terms = get_terms(array('taxonomy' => $taxonomy_filters, 'hide_empty' => true));
		
		if(!is_wp_error($terms) && count($terms) > 0) {
			foreach($terms as $term) {
				// Le term de la taxo est dans le tableau de slug des terms passés en paramètre
				if(!empty($slug_filters)) {
					if(in_array($term->slug, $slug_filters)) {
	...
	public static function compare_meta_values($array_post_meta_values, $meta_query) {
		...
		foreach($array_post_meta_values as $post_meta_value) {
			...
			if(!empty($termData) && isset($termData[$post_meta_value]) && $meta_query['type'] === 'DATE') {
