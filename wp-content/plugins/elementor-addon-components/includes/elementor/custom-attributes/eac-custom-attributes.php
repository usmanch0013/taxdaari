...
class Eac_Custom_Attributes {
	...
	public function eac_attributes_controls_section($element, $section_id, $args) {
		...
		if('section_custom_attributes_pro' === $section_id) {
			...
			$element->start_controls_section('eac_section_attributes',
				[
					'label' => __('EAC attributs personnalisés', 'eac-components'),
			...
			$element->add_control('eac_attributes',
				[
					'label' => __('Ajouter vos attributs', 'eac-components'),
					'type' => Controls_Manager::TEXTAREA,
					'placeholder' => __('Clé|Valeur', 'eac-components'),
					'description' => __("Séparer la clé de sa valeur avec le caractère pipe '|'<br/>Chaque attribut sur une ligne distincte.", "eac-components"),
					'dynamic' => [
						'active' => true,
					],
					'render_type' => 'none',
