...
class Eac_Injection_Elements_Link {
	...
	public function __construct() {
		add_action('elementor/element/column/layout/before_section_end', array($this, 'inject_section_column'), 11, 2);
		add_action('elementor/element/section/section_layout/before_section_end', array($this, 'inject_section_column'), 11, 2);
	...
	public function enqueue_scripts() {
		wp_enqueue_script('eac-element-link', EAC_ADDONS_URL . 'assets/js/elementor/eac-element-link.js',	array('jquery', 'elementor-frontend'), EAC_ADDONS_VERSION, true);
	}
	...
	public function inject_section_column($element, $args) {
		...
		$element->add_control('eac_element_link',
			[
				'label'         => __('EAC Lien container', 'eac-components'),
				'type'          => Controls_Manager::URL,
				'description' => __("Le lien n'est pas actif dans l'éditeur", 'eac-components'),
	...
	public function render_link($element) {
		...
		if(isset($settings['eac_element_link']) && '' !== $settings['eac_element_link']['url']) {
			...
			$element->add_render_attribute('_wrapper', array('data-eac_settings_link' => json_encode($element_settings)));
		}
	}
}
