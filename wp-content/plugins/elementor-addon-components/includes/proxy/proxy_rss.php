...
//if(!isset($_POST['nonce_rss__item']) || !wp_verify_nonce($_POST['nonce_rss__item'], '_nonce_rss__item')) exit;
...
function scrape_rss($urlUser) {
	if(($xml = @file_get_contents($urlUser)) === false) {
		...
		if(preg_match("/(404)/", $error['message'])) {
			...
			echo "\"" . urldecode($match[1]) . "\": " . __("La page demandée n'existe pas.", 'eac-components');
		} else if(preg_match("/(403)/", $error['message'])) {
			preg_match('/\(([^\)]+)\)/', $error['message'], $match);
			echo "\"" . urldecode($match[1]) . "\": " . __("Accès refusé.", 'eac-components');
		} else if(preg_match("/(401)/", $error['message'])) {
			preg_match('/\(([^\)]+)\)/', $error['message'], $match);
			echo "\"" . urldecode($match[1]) . "\": " . __("Non autorisé.", 'eac-components');
		} else if(preg_match("/(503)/", $error['message'])) {
			preg_match('/\(([^\)]+)\)/', $error['message'], $match);
			echo "\"" . urldecode($match[1]) . "\": " . __("Service indisponible. Réessayer plus tard.", 'eac-components');
		} else if(preg_match("/(405)/", $error['message'])) {
			preg_match('/\(([^\)]+)\)/', $error['message'], $match);
			echo "\"" . urldecode($match[1]) . "\": " . __("Méthode non autorisée.", 'eac-components'); 
		} else if(preg_match("/(500)/", $error['message'])) {
			preg_match('/\(([^\)]+)\)/', $error['message'], $match);
			echo "\"" . urldecode($match[1]) . "\": " . __("Erreur Interne du Serveur.", 'eac-components'); 
		} else {
			echo __("HTTP: La requête a échoué.", 'eac-components');
		}
		error_clear_last();
